...
def plotPerColumnDistribution(df1, nGraphShown, nGraphPerRow):
    ...
    for i in range(min(nCol, nGraphShown)):
        ...
        if not np.issubdtype(type(columnDf.iloc[0]), np.number):
            columnDf.value_counts().plot(kind='bar', ax=plt.gca())
        else:
            columnDf.hist(ax=plt.gca())
        plt.title(f"{columnDf.name} distribution")
...
def plotScatterMatrix(df, plotSize, textSize):
    df1 = df.select_dtypes(include=[np.number])  # keep only numerical columns
    df1 = df1.dropna(axis='columns')  # drop columns with NaN values
    df1 = df1[[col for col in df1 if df1[col].nunique() > 1]]  # keep columns with more than 1 unique value
    columnNames = list(df1)
    
    if len(columnNames) > 10:  # limit the number of columns to 10
...
print(X_test)
print(y_test)
def scatterinp(s1, s2, s3, s4, s5):         
    # Your implementation here
    pass

def scatterplt(disea):
    # Debug print to check the value of disea
    print(f"Value of disea: '{disea}'")
    
    try:
        x = ((df.loc[disea]).sum()) # total z sum of symptoms reported for given disease
        x.drop(x[x == 0].index, inplace=True) # dropping symptoms with values 0
        print(x.values)
    except KeyError:
        print(f"KeyError: The disease '{disea}' is not found in the DataFrame.")
#def on_button_click():
    # Get the predicted disease
    #pred1 = StringVar(value=predict_disease())
    #print(f"Predicted disease: {pred1.get()}")
...
conn.commit()
c.execute("SELECT * FROM DecisionTree")
rows = c.fetchall()
c.close() 
conn.close()
